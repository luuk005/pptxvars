...
def _collect_frames(slide):
    """Return list of dicts describing frames in reading order.
       A frame = any shape (or grouped child) whose .name contains 'frame' (case-insensitive).
       Provides absolute left/top for grouped children."""
    frames = []
    ...
    def add_item(shp, left_abs, top_abs, parent_group=None):
        frames.append({
            ...
            "parent_group": parent_group,  # None if slide-level
        })
    ...
    # reading order: top, then left (absolute coords)
    frames.sort(key=lambda x: (int(x["top"]), int(x["left"])))
    return frames


def _add_picture_fit(slide, img_path, left, top, width, height, rotation):
    pic = slide.shapes.add_picture(str(img_path), left, top, width=width)  # keep aspect
    if pic.height > height:
        pic.height = height  # width auto-adjusts
    # center inside original rect
...
def _swap_group_child(slide, group, child, img_path, left_abs, top_abs, width, height, rotation):
    ...
    # Add picture at slide level at absolute coords
    pic = _add_picture_fit(slide, img_path, left_abs, top_abs, width, height, rotation)
    # Place just above the group in z-order so it visually overlays the group
    st = slide.shapes._spTree
    gi = list(st).index(group._element)
    st.remove(pic._element)
    st.insert(gi + 1, pic._element)


def load_plan(yml_path: Path):
    data = yaml.safe_load(yml_path.read_text(encoding="utf-8")) or {}
    slides = data.get("slides") or []
    return [(int(item["index"]), [Path(p) for p in item.get("images", [])]) for item in slides]


def main(pptx_in: Path, plan_yml: Path, pptx_out: Path):
    prs = Presentation(pptx_in)
    plan = load_plan(plan_yml)
    if not plan:
        sys.exit("YAML must define: slides: - index: <int>  images: [paths...]")
    base = plan_yml.parent
    ...
    for idx, imgs in plan:
        ...
        # Apply images in YAML order to frames in reading order
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        sys.exit("Usage: python scripts/swap_frames_ordered.py <in.pptx> <img_mapping.yml> <out.pptx>")
